...
static inline int32_t get_32bit(const uint8_t *data_ptr) {
    // Combine the three bytes into a 32-bit integer
    int32_t raw_data = ((int32_t)data_ptr[2] << 16) | ((int32_t)data_ptr[1] << 8) | (int32_t)data_ptr[0];

    // Handle 20-bit two's complement sign extension
    if (raw_data & 0x80000) {  // Check if the 20th bit (sign bit) is set
        raw_data |= 0xFFF00000;  // Sign-extend to 32 bits
    }
    return raw_data;
}
...
HAL_StatusTypeDef BMP581_Get_CHIPStatus(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_CHIP_STATUS, (uint8_t*)&BMP581_Sensor.CHIP_STATUS.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_DriveConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_DRIVE_CONFIG, (uint8_t*)&BMP581_Sensor.DRIVE_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_DriveConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_DRIVE_CONFIG, (uint8_t*)&BMP581_Sensor.DRIVE_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_INTConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_INT_CONFIG, (uint8_t*)&BMP581_Sensor.INT_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_INTConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_INT_CONFIG, (uint8_t*)&BMP581_Sensor.INT_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_INTSource(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_INT_SOURCE, (uint8_t*)&BMP581_Sensor.INT_SOURCE.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_INTSource(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_INT_SOURCE, (uint8_t*)&BMP581_Sensor.INT_SOURCE.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_FIFOConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_FIFO_CONFIG, (uint8_t*)&BMP581_Sensor.FIFO_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_FIFOConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_FIFO_CONFIG, (uint8_t*)&BMP581_Sensor.FIFO_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_FIFOCount(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_FIFO_COUNT, (uint8_t*)&BMP581_Sensor.FIFO_COUNT.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_FIFOFrameSel(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_FIFO_SEL, (uint8_t*)&BMP581_Sensor.FIFO_SEL.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_FIFOFrameSel(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_FIFO_SEL, (uint8_t*)&BMP581_Sensor.FIFO_SEL.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_TempPressData(BMP581_sensor_data_t *data) {
    ...
    if (ReadRegister(BMP581_ADDRESS, BMP581_REG_TEMP_DATA_XLSB, reg_data, 6, &hi2c2) == HAL_OK) {
        // Convert raw temperature data
        int32_t raw_temp = get_32bit(&reg_data[0]);  // Extract temperature (first 3 bytes)
        data->temperature = (float)raw_temp / TEMP_COEFF;

        // Convert raw pressure data
        int32_t raw_press = get_32bit(&reg_data[3]);  // Extract pressure (next 3 bytes)
        data->pressure = (float)raw_press / PRESS_COEFF;
...
HAL_StatusTypeDef BMP581_Get_INTStatus(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_INT_STATUS, (uint8_t*)&BMP581_Sensor.INT_STATUS.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_Status(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_STATUS, (uint8_t*)&BMP581_Sensor.STATUS.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_FIFOData(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_FIFO_DATA, (uint8_t*)&BMP581_Sensor.FIFO_DATA, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_NVMAddr(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_NVM_ADDR, (uint8_t*)&BMP581_Sensor.NVM_ADDR.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_NVMAddr(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_NVM_ADDR, (uint8_t*)&BMP581_Sensor.NVM_ADDR.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_NVMData(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_NVM_DATA_LSB, (uint8_t*)&BMP581_Sensor.NVM_DATA, TWO, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_NVMData(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_NVM_DATA_LSB, (uint8_t*)&BMP581_Sensor.NVM_DATA, TWO, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_DSPConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_DSP_CONFIG, (uint8_t*)&BMP581_Sensor.DSP_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_DSPConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_DSP_CONFIG, (uint8_t*)&BMP581_Sensor.DSP_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_DSPIIRConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_DSP_IIR, (uint8_t*)&BMP581_Sensor.DSP_IIR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_DSPIIRConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_DSP_IIR, (uint8_t*)&BMP581_Sensor.DSP_IIR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_OOR_PRESS_THR(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_OOR_THR_P_LSB, (uint8_t*)&BMP581_Sensor.OOR_PRESS_THR, TWO, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_OOR_PRESS_THR(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_OOR_THR_P_LSB, (uint8_t*)&BMP581_Sensor.OOR_PRESS_THR, TWO, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_OORRange(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_OOR_RANGE, (uint8_t*)&BMP581_Sensor.OOR_PRESS_RNG, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_OORRange(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_OOR_RANGE, (uint8_t*)&BMP581_Sensor.OOR_PRESS_RNG, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_OORConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_OOR_CONFIG, (uint8_t*)&BMP581_Sensor.OOR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_OORConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_OOR_CONFIG, (uint8_t*)&BMP581_Sensor.OOR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_OSRConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_OSR_CONFIG, (uint8_t*)&BMP581_Sensor.OSR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_OSRConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_OSR_CONFIG, (uint8_t*)&BMP581_Sensor.OSR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Set_ODRConfig(void) {
    return WriteRegister(BMP581_ADDRESS, BMP581_REG_ODR_CONFIG, (uint8_t*)&BMP581_Sensor.ODR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_ODRConfig(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_ODR_CONFIG, (uint8_t*)&BMP581_Sensor.ODR_CONFIG.Val, ONE, &hi2c2);
}
...
HAL_StatusTypeDef BMP581_Get_OSREff(void) {
    return ReadRegister(BMP581_ADDRESS, BMP581_REG_OSR_EFF, (uint8_t*)&BMP581_Sensor.OSR_EFF.Val, ONE, &hi2c2);
}
