...
HAL_StatusTypeDef WriteRegister(uint8_t device_address, uint8_t reg_address, uint8_t* data, uint16_t len, I2C_HandleTypeDef* hi2c) {
        ...
        HAL_StatusTypeDef status = HAL_I2C_Mem_Write_DMA(hi2c, device_address, reg_address, I2C_MEMADD_SIZE_8BIT, data, len);
        if (status != HAL_OK) {
            return status;
        }
        ...
        return HAL_OK;
    #else
        return HAL_I2C_Mem_Write(hi2c, device_address, reg_address, I2C_MEMADD_SIZE_8BIT, data, len, I2C_TIMEOUT);
    #endif
}
...
HAL_StatusTypeDef ReadRegister(uint8_t device_address, uint8_t reg_address, uint8_t* data, uint16_t len, I2C_HandleTypeDef* hi2c) {
    #ifdef USE_DMA
        volatile uint8_t transfer_complete = 0;
        HAL_StatusTypeDef status = HAL_I2C_Mem_Read_DMA(hi2c, device_address, reg_address, I2C_MEMADD_SIZE_8BIT, data, len);
        if (status != HAL_OK) {
            return status;
        }
        ...
        return HAL_OK;
    #else
        return HAL_I2C_Mem_Read(hi2c, device_address, reg_address, I2C_MEMADD_SIZE_8BIT, data, len, I2C_TIMEOUT);
    #endif
}
...
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef* hi2c) {
    // Set a flag or handle any custom logic needed when DMA transfer completes
    // This flag should be made external if needed in other parts of the code
...
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef* hi2c) {
    // Handle I2C error scenarios
    extern volatile uint8_t transfer_complete;
    transfer_complete = 1;  // End transfer on error
}
#endif
/* USER CODE END 1 */
